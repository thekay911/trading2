...
| "M1 은 스프레드에 죽는다" | `max_spread_ratio` — 스프레드를 절대값이 아니라 **손절 폭 대비 비율**로 본다 |
...
## 3. 프리셋 = 채널의 계좌 분할 (전부 XAUUSD 기준)

| 프리셋 | 대응 | 리스크 | 타임프레임 | 손절 폭 | 근거 |
|---|---|---|---|---|---|
| `swing` | 스윙 계좌 | 1% | D1 → H4 → H1 | $6~60 | "H1 스윙은 15-30일 유효" |
| `intraday` | (추가) | 1% | H4 → H1 → M15 | $2.5~25 | 금 변동성에 가장 맞는 프레임 |
| `scalp` | 스캘핑 계좌 | 0.5% | H1 → M15 → M5 | $1.5~10 | "M15 로 하루 5-8 시그널" |
| `highrisk` | 소액 고위험 계좌 | 6% | M15 → M5 → M1 | $0.8~4 | "M1 은 SL 이 2-3핍이라 스프레드에 죽는다 → 작은 계좌로만" |

`intraday` 는 채널에 대응하는 이름이 없는, 이 저장소가 추가한 프리셋이다.
채널이 실제로 가장 많이 보여 주는 금 스윙/데이 셋업(H1~H4 구조에 M15 진입)이
기존 두 프리셋 사이에 비어 있어서 채웠다.

손절 폭 가드(`min_sl_price`/`max_sl_price`)는 채널 원문에 직접 대응하는 규칙은 아니고,
"M5 셋업에 $40 손절" 같은 헛발질을 걸러내기 위해 금 변동성에 맞춰 넣은 안전장치다.
자세한 근거는 [GOLD.md](GOLD.md) 참고.
